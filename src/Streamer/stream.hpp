...
public:
    ...
    virtual StatPacket getPacket() = 0;
};
...
private:
    rtc::synchronized_callback<StreamSourceType, uint64_t, rtc::binary, StatPacket> sampleHandler;
...
public:
    void onSample(std::function<void(StreamSourceType, uint64_t, rtc::binary, StatPacket)> handler);
    void start();
    void stop();
